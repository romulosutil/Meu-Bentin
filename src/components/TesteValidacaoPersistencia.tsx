...
const TesteValidacaoPersistencia = () => {
  ...
  const [tests, setTests] = useState<PersistenceTest[]>([
    ...
    },
    {
      ...
      description: 'Verificar se capital de giro é persistido no Supabase',
      status: 'pending'
    }
  ]);
  ...
  const testCapitalGiroPersistence = async () => {
    updateTestStatus('Capital de Giro', 'running');
    try {
      console.log('🧪 Testando persistência de capital de giro...');
      
      const { supabaseService } = await import('../utils/supabaseServiceSemVendedor');
      const capitalGiro = await supabaseService.getCapitalGiro();
      
      if (capitalGiro) {
        updateTestStatus('Capital de Giro', 'success', undefined, {
          count: 1,
          sample: [`R$ ${capitalGiro.valorInicial.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`]
        });
      } else {
        throw new Error('Nenhum capital de giro configurado');
      }
      
    } catch (error) {
      console.error('❌ Erro no teste de capital de giro:', error);
      updateTestStatus('Capital de Giro', 'error', error instanceof Error ? error.message : 'Erro desconhecido');
    }
  };
  ...
  const runAllTests = async () => {
    ...
    try {
      ...
      await testCapitalGiroPersistence();
