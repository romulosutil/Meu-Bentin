// =====================================================
// COMPONENTE GERENCIAR CLIENTES - COM MÁSCARAS DE INPUT
// =====================================================
// Implementa todas as máscaras de input solicitadas:
// 1. Telefone: (XX) XXXXX-XXXX
// 2. Data: DD/MM/AAAA
// 3. Instagram: @usuario
// 4. Preserva compatibilidade com dados existentes
// =====================================================
...
interface GerenciarClientesComMascarasProps {
  open: boolean;
  onCancel: () => void;
}
...
const FormularioClienteComMascaras: React.FC<{
  ...
  const handleInstagramChange = useCallback((value: string) => {
    ...
    setFormData(prev => ({ ...prev, instagram: value.startsWith('@') ? value : `@${value}` }));
  }, []);
...
/**
 * Modal de Gerenciamento de Clientes COM MÁSCARAS
 */
const GerenciarClientesComMascaras: React.FC<GerenciarClientesComMascarasProps> = ({
  open,
  onCancel
}) => {
  const {
  ...
  // Handlers CORRIGIDOS COM VERIFICAÇÃO DE INTEGRAÇÃO
                ...
                {stats && (
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <StatsCard
                      ...
                      trend={stats.totalClientes > 0 ? 'up' : 'neutral'}
                    ...
                    <StatsCard
                      ...
                      trend={stats.clientesAtivos > 0 ? 'up' : 'neutral'}
                      description="Com compras recentes"
                    />
                    <StatsCard
                      title="Total de Filhos"
                      value={stats.totalFilhos.toString()}
                      icon={<Baby className="h-4 w-4" />}
                      trend={stats.totalFilhos > 0 ? 'up' : 'neutral'}
                      description="Dependentes cadastrados"
                    />
                    <StatsCard
                      title="Média de Filhos"
                      value={stats.mediaFilhosPorCliente.toFixed(1)}
...
export default GerenciarClientesComMascaras;