...
import { CurrencyInput } from './ui/masked-input';
...
const CaixaComMascaras = () => {
  ...
  // Estados do formulário com máscara de moeda
  const [formData, setFormData] = useState({
    tipo: '',
    valor: 0, // Agora é number para trabalhar com CurrencyInput
    descricao: ''
  });
  ...
  // Handler para mudança de valor com máscara
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.tipo || formData.valor <= 0) {
      toast({
        title: "Erro",
        description: "Tipo e valor são obrigatórios, e valor deve ser maior que zero.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      const novaMovimentacao = {
        ...
        descricao: formData.descricao.trim() || `Movimentação de ${formData.tipo} - ${formatarMoeda(formData.valor)}`
      };
      ...
      if (resultado.sucesso) {
        ...
        // Limpar formulário
        setFormData({ tipo: '', valor: 0, descricao: '' });
    ...
    <TooltipProvider>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Formulário de Nova Movimentação COM MÁSCARA */}
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="descricao"
                    placeholder="Descreva a movimentação (será gerada automaticamente se vazio)"
...
export default CaixaComMascaras;