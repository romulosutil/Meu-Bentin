...
interface FormularioProdutoResponsivoProps {
  open: boolean;
  produto?: Produto;
...
const FormularioProdutoResponsivo: React.FC<FormularioProdutoResponsivoProps> = ({
  open,
  produto,
  onSubmit,
  onCancel,
  ...
  useEffect(() => {
    const carregarDados = async () => {
      try {
        // Configurações padrão - método getConfiguracoesProductos não implementado
        // const configs = await supabaseService.getConfiguracoesProductos();
        // setConfiguracoes(configs);
        console.log('Usando configurações padrão para produtos');
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="modal-container relative bg-white rounded-xl shadow-xl w-[90vw] max-w-5xl flex flex-col border border-gray-200" style={{ maxHeight: '90vh', overflow: 'hidden' }}>
        ...
        {/* Conteúdo scrollável com melhor estrutura */}
        <div className="modal-body bentin-scroll p-6 space-y-6" style={{ flexGrow: 1, overflowY: 'auto' }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* Coluna 1 - Informações Básicas e Preços */}
                    ...
                    <div className="space-y-4">
                      ...
                      {isUploading && (
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-bentin-pink h-2 rounded-full transition-all duration-300"
                          ...
                          >
                            {configuracoes.generosPadrao.map((genero) => (
                              <div key={genero} className="flex items-center space-x-2">
                                <RadioGroupItem value={genero} />
                                <Label className="capitalize text-sm">{genero}</Label>
                              </div>
                            ))}
                          </RadioGroup>
                        </FormField>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {configuracoes.tamanhosPadrao.map(tamanho => (
                              <Button
                                ...
                                className={`h-8 px-3 text-sm ${formData.tamanhos?.includes(tamanho) ? "bg-bentin-blue" : ""}`}
                              >
                                {tamanho}
                              </Button>
                            ))}
                          ...
                          <div className="flex flex-wrap gap-2">
                            {configuracoes.coresPadrao.map(cor => (
                              <Button
                                ...
                                className={`h-8 px-3 text-sm ${formData.cores?.includes(cor) ? "bg-bentin-green" : ""}`}
                              >
                                {cor}
                              </Button>
                            ))}
                            ...
                            <div className="flex flex-wrap gap-2 items-center">
                              ...
                              {formData.cores.map(cor => (
                                <Badge 
                                  key={cor} 
                                  variant="outline"
                                  className="flex items-center gap-1 border-bentin-green text-bentin-green"
                                >
                                  {cor}
                                  <X 
                                    className="h-3 w-3 cursor-pointer hover:text-red-600" 
        ...
        <div className="modal-footer p-6 border-t border-gray-200 bg-white" style={{ flexShrink: 0 }}>
          <div className="flex gap-3 justify-end">
            ...
            <Button 
              ...
              disabled={isSubmitting}
              className="px-6 bg-bentin-pink hover:bg-bentin-pink/90"
            >
...
export default FormularioProdutoResponsivo;